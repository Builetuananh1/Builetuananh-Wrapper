...
static void error_sysegl() {
    fprintf(stderr, "Builetuananh Init: Failed to load system EGL: %s", dlerror());
    abort();
}

static void error_init(const char* functionName) {
    fprintf(stderr, "Builetuananh Init: Failed to load function \"%s\"", functionName);
    abort();
}
...
__attribute__((constructor, used)) void proc_init(){
    ...
    if(eglHandle == NULL){
        printf("Builetuananh Init: failed loading custom libEGL, using default\n");
...
#define GLESOVERRIDE(name)                                        \
    if(!strcmp(procname, #name)) {                                \
        fprintf(stderr, "Builetuananh: Overridden %s\n", #name);                        \
