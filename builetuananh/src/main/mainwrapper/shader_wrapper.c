...
GLuint glCreateProgram(void) {
    ...
    if(prog_info == NULL) {
        printf("Builetuananh ShdrWp: failed to allocate program_info\n");
        abort();
    }
...
static void insert_fragout_pos(char* source, int* size, const char* name, GLuint pos) {
    ...
    snprintf(src_string, sizeof(src_string), "/* BUILETUANANH INSERT LOCATION %s BUILETUANANH */", name);
...
void glLinkProgram(GLuint program) {
    ...
    if(shader == NULL) {
        printf("Builetuananh ShdrWp: failed to patch frag data location due to missing shader info\n");
        goto fallthrough;
    }
    ...
    if(patched_shader == 0) {
        free(new_source);
        printf("Builetuananh ShdrWp: failed to initialize patched shader\n");
        goto fallthrough;
    }
    ...
    if(compileStatus != GL_TRUE) {
        ...
        printf("Builetuananh ShdrWp: failed to compile patched fragment shader, using default. Log:\n\n%s\n\nShader content:\n\n%s\n\n", log, const_source);
        goto fallthrough;
    }
...
GLuint glCreateShader(GLenum shaderType) {
    ...
    if(info_struct == NULL) {
        printf("Builetuananh ShdrWp: failed to allocate shader_info\n");
        abort();
    }
...
void glShaderSource(GLuint shader, GLsizei count, const GLchar *const*string, const GLint *length) {
    ...
    shader_info_t* shader_info = unordered_map_get(current_context->shader_map, (void*)shader);
    if(shader_info == NULL) {
        printf("Builetuananh ShdrWp: shader_info missing for shader %u\n", shader);
...
#undef SRC_LEN
    ...
    if(!new_source) {
        printf("Builetuananh ShdrWp: failed to convert&optimize shader %u, skipping\n", shader);
        goto end;
    } else {
