...
void basevertex_init(context_t* context) {
    ...
    if(context->drawelementsbasevertex != NULL) {
        printf("Builetuananh: BaseVertex render calls will use the host driver implementation\n");
        return;
    }
    if(!context->es31) {
        printf("Builetuananh: BaseVertex render calls not available: requires OpenGL ES 3.1\n");
        return;
    }
    es3_functions.glGenBuffers(1, &renderer->indirectRenderBuffer);
    GLenum error = es3_functions.glGetError();
    if(error != GL_NO_ERROR) {
        printf("Builetuananh: Failed to initialize indirect buffers: %x\n", error);
        return;
    }
    renderer->ready = true;
}
...
void glDrawElementsBaseVertex(GLenum mode, GLsizei count, GLenum type, const void *indices, GLint basevertex) {
    ...
    if(elementbuffer == 0) {
        printf("Builetuananh: Base vertex draws without element buffer are not supported\n");
        return;
    }
    GLint typeBytes = type_bytes(type);
    uintptr_t indicesPointer = (uintptr_t)indices;
    if(indicesPointer % typeBytes != 0) {
        printf("Builetuananh: misaligned base vertex draw not supported\n");
    }
...
void glMultiDrawElementsBaseVertex(GLenum mode,
    ...
    for(GLsizei i = 0; i < drawcount; i++) {
        ...
        if(indicesPointer % typeBytes != 0) {
            printf("Builetuananh: misaligned base vertex draw not supported (draw %i)\n", i);
            return;
        }
