...
INTERNAL bool filter_params_float(GLenum target, GLenum pname, GLfloat param) {
    if(pname == GL_TEXTURE_LOD_BIAS) {
        if(param != 0.0f) {
            ...
            if(!lodbias_trigger) {
                printf("Builetuananh: setting GL_TEXTURE_LOD_BIAS to nondefault value not supported\n");
            }
        }
        return false;
    }
...
void glTexParameterIiv( 	GLenum target,
    ...
    if(pname != GL_TEXTURE_SWIZZLE_RGBA) {
        if(!trigger_gltexparameteri) {
            printf("Builetuananh: glTexParameterIiv for parameters other than GL_TEXTURE_SWIZZLE_RGBA is not supported\n");
...
void glTexParameterIuiv( 	GLenum target,
    ...
    if(pname != GL_TEXTURE_SWIZZLE_RGBA) {
        if(!trigger_gltexparameteri) {
            printf("Builetuananh: glTexParameterIuiv for parameters other than GL_TEXTURE_SWIZZLE_RGBA is not supported\n");
            trigger_gltexparameteri = true;
        }
        return;
    }
...
const GLubyte* glGetString(GLenum name) {
    ...
    switch(name) {
        case GL_VERSION:
            return (const GLubyte*)"OpenGL 3.3.0 Builetuananh-Wrapper 0.0.1";
        case GL_SHADING_LANGUAGE_VERSION:
            return (const GLubyte*)"4.60 Builetuananh";
        case GL_VENDOR:
            return (const GLubyte*)"Builetuananh";
        case GL_EXTENSIONS:
...
void glGetIntegerv(GLenum pname, GLint* data) {
    ...
    switch (pname) {
		...
		case GL_CONTEXT_PROFILE_MASK:
			// Builetuananh is always core profile
...
void glTexBuffer(GLenum target, GLenum internalFormat, GLuint buffer) {
    ...
    if(current_context->es32) es3_functions.glTexBuffer(target, internalFormat, buffer);
        ...
        printf("Builetuananh: Buffer textures aren't supported on your device\n");
    }
}
...
__attribute((constructor)) void init_noerror() {
    ...
    debug = env_istrue("BUILETUANANH_DEBUG");
    never_flush_buffers = env_istrue_d("BUILETUANANH_NEVER_FLUSH_BUFFERS", true);
    coherent_dynamic_storage = env_istrue_d("BUILETUANANH_COHERENT_DYNAMIC_STORAGE", true);
    if(!noerror) printf("Builetuananh will NOT ignore GL errors. This may break mods, consider yourself warned.\n");
    if(coherent_dynamic_storage) printf("Builetuananh will force dynamic storage buffers to be coherent.\n");
    if(debug) printf("Builetuananh will allow GL_DEBUG_OUTPUT to be enabled. Expect massive logs.\n");
    if(never_flush_buffers) printf("Builetuananh will prevent all explicit buffer flushes.\n");
}
