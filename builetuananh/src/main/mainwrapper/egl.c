...
void build_extension_string(context_t* context) {
    ...
    if(context->buffer_storage) {
        if(!env_istrue("BUILETUANANH_HIDE_BUFFER_STORAGE"))
            add_extra_extension(context, &length, "GL_ARB_buffer_storage");
        else printf("Builetuananh: The buffer storage extension is hidden.\n");
    }
...
static void find_esversion(context_t* context) {
    ...
    printf("Builetuananh: Running on OpenGL ES %i.%i with ESSL %i\n", esmajor, esminor, context->shader_version);
...
#undef SET_FUNC
    ...
    return;
    fail:
    printf("Builetuananh: Failed to detect OpenGL ES version");
}
...
context_t* builetuananh_get_current_context(void) {
